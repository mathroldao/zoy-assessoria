...
.profile-card { border-radius:24px; padding:24px; margin-bottom:16px; }
.creator-card { border-radius:18px; padding:14px; margin-bottom:10px; }
.creator-card-active { background:#F8F4FF; border:1px solid #DCCBFF; border-radius:18px; padding:14px; margin-bottom:10px; }
...
if menu == "Dashboard":
    ...
    for col, (title, number) in zip(
        [c1, c2, c3, c4],
        [("Creators ativos", len(st.session_state.creators)), ("Oportunidades abertas", open_opps), ("Fechamentos do mês", closed_opps), ("Reuniões da semana", 6)]
    ):
        with col:
                ...
                if st.form_submit_button("Salvar influenciador"):
                    if not nome:
                        ...
                        st.session_state.creators[nome] = {
                            "nome": nome, "nome_artistico": nome_artistico or nome, "initials": initials_from_name(nome),
                            "handle": handle, "nicho": nicho, "cidade": cidade, "status": status, "email": email,
                            "telefone": telefone, "aniversario": aniversario, "cpf_cnpj": cpf_cnpj, "endereco": endereco,
                            "pix": pix, "banco": banco, "agencia": agencia, "conta": conta, "responsavel": "Jean",
                            "bio": bio, "posicionamento": posicionamento, "tom_voz": tom_voz,
                            "marcas_sonho": "", "marcas_no_fit": "", "obs": obs, "seguidores": "0", "alcance": "0",
                            "impressoes": "0", "stories": "0", "engajamento": "0%", "crescimento": "0%",
    ...
    with left:
        ...
        shown_names = [n for n in names if query in n.lower() or query in st.session_state.creators[n].get("handle", "").lower()]
            ...
            st.markdown(
                ...
                <div class="{card_class}">
                    ...
                    <div style="display:inline-block; vertical-align:middle;">
                        <b>{name}</b><br>
                        <span class="muted">{data.get("handle", "")}</span><br>
                        <span class="muted">{status_badge}</span>
                    </div>
                </div>
                ''',
                unsafe_allow_html=True
        ...
        if st.session_state.editing_creator == selected_creator:
            with st.expander("Editar dados do influenciador", expanded=True):
                with st.form("form_edit_creator"):
                    ...
                    if save:
                        updated = {
                            **creator, "nome": edit_nome, "nome_artistico": edit_nome_artistico,
                            "initials": initials_from_name(edit_nome), "handle": edit_handle, "email": edit_email,
                            "telefone": edit_telefone, "cidade": edit_cidade, "nicho": edit_nicho,
                            "aniversario": edit_aniversario, "cpf_cnpj": edit_cpf_cnpj, "status": edit_status,
                            "pix": edit_pix, "banco": edit_banco, "agencia": edit_agencia, "conta": edit_conta,
                            "endereco": edit_endereco, "bio": edit_bio, "posicionamento": edit_posicionamento,
                            "tom_voz": edit_tom_voz, "marcas_sonho": edit_marcas_sonho,
                            "marcas_no_fit": edit_marcas_no_fit, "obs": edit_obs, "responsavel": "Jean",
                        }
                        if edit_nome != selected_creator:
                            del st.session_state.creators[selected_creator]
                            st.session_state.creators[edit_nome] = updated
                            st.session_state.selected_creator = edit_nome
                        else:
    ...
    with col1:
        ...
        objetivo = st.selectbox("Objetivo do mês", ["Crescimento", "Monetização", "Posicionamento", "Autoridade", "Relacionamento com marcas", "Awareness"])
        pilares = st.multiselect("Pilares editoriais", ["Lifestyle", "Beleza", "Skincare", "Humor", "Moda", "Viagem", "Fitness", "Gastronomia"], default=["Lifestyle"])
...
compile(code, "app_zoy_assessoria_lista_final.py", "exec")
