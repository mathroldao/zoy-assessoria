...
campaigns_df = pd.DataFrame([
    {"Marca": "Adidas", "Campanha": "Always On", "Entrega": "3 Reels + 6 Stories", "Prazo": "25/05", "Status": "Produção"},
    {"Marca": "Lipton", "Campanha": "Verão 2025", "Entrega": "2 Stories + 1 Reel", "Prazo": "30/05", "Status": "Aprovação"},
    {"Marca": "FINI", "Campanha": "Doces Momentos", "Entrega": "2 Reels + 4 Stories", "Prazo": "15/06", "Status": "Negociação"},
])
...
if menu == "Dashboard":
    ...
    with p1:
        st.markdown('<div class="card"><h4>FINI</h4><p class="muted">Campanha de Reels + Stories</p><span class="pill">Negociação</span></div>', unsafe_allow_html=True)
    with p2:
        st.markdown('<div class="card"><h4>Adidas</h4><p class="muted">Campanha Always On</p><span class="pill">Produção</span></div>', unsafe_allow_html=True)
    with p3:
        st.markdown('<div class="card"><h4>Lipton</h4><p class="muted">Stories + Reel</p><span class="pill">Aprovação</span></div>', unsafe_allow_html=True)
...
# =========================
    ...
    with left:
        ...
        st.markdown(f'<div class="muted" style="margin-top:22px;">{len(creators_data)} influenciadores cadastrados</div>', unsafe_allow_html=True)
    ...
    with right:
        ...
        with tabs[0]:
            ...
            with col_a:
                st.markdown("### Sobre o influenciador")
                st.markdown("**E-mail**")
                st.write(creator["email"])
                st.markdown("**Telefone**")
                st.write(creator["telefone"])
                st.markdown("**Nicho**")
                st.write(creator["nicho"])
                st.markdown("**Aniversário**")
                st.write(creator["aniversario"])
                st.markdown("**Posicionamento**")
                st.write(creator["posicionamento"])
                st.markdown("**Observações**")
                st.write(creator["obs"])

                tags_html = "".join([f'<span class="pill">{tag}</span>' for tag in creator["tags"]])
            ...
            with col_b:
                ...
                with m1:
                    st.metric("Seguidores", creator["seguidores"], "2,3%")
                with m2:
                    st.metric("Alcance médio", creator["alcance"], "8,1%")
                with m3:
                    st.metric("Views Stories", creator["stories"], "5,2%")
                with m4:
                    st.metric("Engajamento", creator["engajamento"], "0,6%")
        ...
        with tabs[1]:
            ...
            st.info("Aqui vamos registrar seguidores, alcance, impressões, views, engajamento e atualização manual das métricas.")
        ...
        with tabs[3]:
            st.markdown("### Acompanhamento de postagem")
            acompanhamento_df = pd.DataFrame([
                {"Campanha": "Adidas", "Entrega": "Reel 1", "Data prevista": "25/05", "Status": "Aguardando postagem", "Link": ""},
                {"Campanha": "Lipton", "Entrega": "Stories", "Data prevista": "30/05", "Status": "Aguardando aprovação", "Link": ""},
                {"Campanha": "FINI", "Entrega": "Reel 1", "Data prevista": "15/06", "Status": "Em produção", "Link": ""},
            ])
            st.dataframe(acompanhamento_df, use_container_width=True, hide_index=True)

        with tabs[4]:
            st.markdown("### Histórico")
            st.write("18/05 — Reunião mensal realizada")
            st.write("15/05 — Cliente Adidas pediu disponibilidade")
            st.write("10/05 — Ajuste de posicionamento aprovado")

        with tabs[5]:
            st.markdown("### Documentos")
            st.info("Contratos, mídia kit, dados bancários, NFs e arquivos importantes.")
...
# =========================
    ...
    with c1:
        st.markdown('<div class="card"><div class="section-title">Briefing recebido</div><p>Sephora</p><p>C&A</p></div>', unsafe_allow_html=True)
    with c2:
        st.markdown('<div class="card"><div class="section-title">Negociação</div><p>FINI</p><p>Amaro</p></div>', unsafe_allow_html=True)
    with c3:
